...
func putHandler(c *gin.Context, db *gorm.DB) {
	...
	// if c.Bind(&newStudent) == nil {
	// 	_, err := db.Exec("update students set student_name=$1, student_age=$2, student_address=$3, student_phone_no=$4 where student_id=$5", newStudent.Student_name, newStudent.Student_age, newStudent.Student_address, newStudent.Student_phone_no, studentId)
	// 	if err != nil {
	// 		c.JSON(http.StatusInternalServerError, gin.H{"message": err.Error()})
	// 	} else {
	...
	c.JSON(http.StatusOK, gin.H{"message": "success update", "data": reqStudent})
